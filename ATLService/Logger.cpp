...
//#endif // _DEBUG
	if (GetLog(pFileName) == S_OK)
	{
		...
		//{
		auto iLogLevel = logging::trivial::debug;
		...
		boost::shared_ptr< sink_t > sinkEventlog(new sink_t());

		sinkEventlog->set_formatter
		(
			expr::format("%1%: [%2%] - %3%")
		...
		sinkEventlog->locked_backend()->set_event_type_mapper(mapping);
		
		logging::core::get()->add_sink(sinkEventlog);
